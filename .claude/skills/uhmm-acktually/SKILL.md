---
name: uhmm-acktually
description: Log what we did this session to plain-text files in the repo (uhmm-acktually-logs/), and refresh the rolling project recap. Invoke as /uhmm-acktually whenever you want to capture progress so a future session can get up to speed fast. The live site shows these logs via a dev-only button.
---

# uhmm-acktually

Keep a lightweight, human-readable record of what we do, stored as plain text
**in the project itself** so any future session (or the dev-only button on
the live site) can get up to speed in seconds.
...
1. **Find / create the current session file.**
   ...
   - Note: a Stop hook may already be appending entries to this file automatically
...
- If the user invokes this at the *end* of a session, make sure the final
  bullets and the summary reflect everything done.
